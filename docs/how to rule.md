...
Самая базовая единица движка - правило.
...
При сохранении любого `.json` файла в `rules/` сервер автоматически перезагружает правила без перезапуска. Если новые правила не компилируются, то сервер продолжает работать со старой версией.
...
Если `SNAPSHOT_PATH` не задан, то сервер падает при старте с понятной ошибкой.
